...
const RecipesRestaurant = () => {
    ...
    const getData = () => {
        ...
        const notAtPageBottom = (windowHeight + scrollTop) < fullHeight;
        const apiCall = resContainerVisibility && footerDataVisibility && payloadForRecipes.widgetOffset.collectionV5RestaurantListWidget_SimRestoRelevance_food && notAtPageBottom;
        ...
        <div className="flex flex-wrap justify-center" ref={resContainerRef}>
            {
                resList?.map((restaurant, idx) => idx > 2 && <Link style={{ color: "black", textDecoration: 'none' }} key={restaurant?.card?.card?.info?.id} to={"/restaurant/" + restaurant?.card?.card?.info?.id}>
                    {
                        ("aggregatedDiscountInfoV3" in restaurant?.card?.card?.info) ? <RestaurantCardOffer resObj={restaurant?.card?.card} /> : <RestaurantCard resObj={restaurant?.card?.card} />
                    }
                </Link>)
            }
        </div>
